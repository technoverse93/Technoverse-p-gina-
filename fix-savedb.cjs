...
const regex = /export async function saveDB\(newDb: Database\) \{[\s\S]*?\}\n\n/g;
...
const newSaveDB = `export async function saveDB(newDb: Database) {
  ...
  try {
    ...
    // We don't need to re-merge into localCache here because we already updated it optimistically.
    // Concurrent onSnapshot updates from other devices will overwrite localCache as appropriate,
    // which is perfectly fine for a real-time system.
  } catch (err) {
...
code = code.replace(regex, newSaveDB);
