...
function notifyProductSyncError(message: string) {
...
async function syncProductsToSupabase(added: Product[], modified: Product[], deleted: Product[]) {
  ...
  for (const item of added) {
    ...
    if (error) errors.push(`Error creando producto "${item.name}": ${error.message}`);
  }
  for (const item of modified) {
    const { error } = await supabase.from('products').update(mapProductToSupabase(item)).eq('id', item.id);
    if (error) errors.push(`Error actualizando producto "${item.name}": ${error.message}`);
  }
  for (const item of deleted) {
    const { error } = await supabase.from('products').delete().eq('id', item.id);
    if (error) errors.push(`Error eliminando producto "${item.name}": ${error.message}`);
  }
  if (errors.length > 0) {
    const message = errors.join(' | ');
    notifyProductSyncError(message);
    // Re-sincronizamos con lo que realmente quedó en Supabase para no dejar
    // productos "fantasma" que solo existen en la pantalla del administrador.
    await refreshProductsFromSupabase();
...
export async function saveLogo(base64: string) {
  try {
    ...
    checkQuotaError(err);
    // Antes este error se ignoraba y el logo solo quedaba en memoria (localCache),
    // dando la falsa impresión de éxito hasta el próximo refresh. Ahora se
    // relanza para que la pantalla de administración avise que NO se guardó.
